...
export function Overview() {
  ...
  const fetchViewsToday = async () => {
    try {
      const { data } = await api.api.analytics["views-today"].get();
      if (data && typeof data.viewsToday === "number") {
        setViewsToday(data.viewsToday);
      }
    } catch (err) {
      console.error("Failed to fetch views today:", err);
    }
  };
  ...
  const refreshData = async () => {
    await Promise.all([fetchCargas(), fetchViewsToday()]);
  };
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <ScrapeButton onComplete={refreshData} />
        </div>
      </div>
